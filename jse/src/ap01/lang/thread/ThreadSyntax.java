 ...
 * 2. Runnalble 인터페이스를 구현하는 방법
 * 객체지향적인 방법은 인터페이스 구현방법을 취하는 게 낫다.
 * Runnalble 구현방법을 추천
 */
/*
 * 스레드 콜백 메소드
 * start() : JVM 이 스케쥴 상 여유가 생겼을때, 스레드 스케쥴상 run() 을 호출
 * run() : 스레드 상태에서 처리하려고 하는 모든 비지니스 로직이 구현됨
 * sleep(long milisecond) : 지정된 시간동안 스레드를 쉬게 하고,
 *     그 시간이 지나면 다시 작동한다. 1000 분의 1초
 * wait() : 현재 스레드 무한대기, notify(), notifyAll() 메소드로 재실행시킴
 * suspend() : 스레드 실행권한을 다른 스레드에 넘김
 * stop() : 스레드 실행을 종료
 */
/*단일 스레드
